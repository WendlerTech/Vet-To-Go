...
{
    public partial class FrmLicensedAnimals : Form
    {
        ...
        private void RetrieveData(int speciesID)
        {
            ...
            {
                MessageBox.Show("An error occured while attempting to retrieve unlicensed animals. Namely:\n" + ex.Message, "SQL Exception",
        ...
        private String GetQueryString(int speciesID)
        {
            ...
            String queryString = "SELECT T1.CustomerID, T1.AnimalName, T1.AnimalBreed, T2.FirstName, T2.LastName FROM Tbl_Animal T1 " +
                "INNER JOIN Tbl_Customer T2 ON T1.CustomerID = T2.CustomerID WHERE T1.IsLicensed = 0;";
            if (speciesID != 0)
            {
                queryString = "SELECT T1.CustomerID, T1.AnimalName, T1.AnimalBreed, T2.FirstName, T2.LastName FROM Tbl_Animal T1 " +
                    "INNER JOIN Tbl_Customer T2 ON T1.CustomerID = T2.CustomerID WHERE T1.IsLicensed = 0 AND T1.SpeciesID = @speciesID;";
            }
